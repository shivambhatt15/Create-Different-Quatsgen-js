...
document.querySelector("#quatgen").addEventListener("click", () => {
quatgen();
});

function quatgen(){
// grab all the keys in the dictionary (authors) and store in an array
const authors = Object.keys(quaget);
// grab a random key (author) and store it in author
const author = authors[Math.floor(Math.random() * authors.length)];
// grab the value(quote) that belongs to that key
const quote = quaget[author]

document.querySelector("#quoteone").textContent = quote;
document.querySelector("#authorone").textContent = author;
}


// What is a querySelector in JavaScript?
// The querySelector() is a method used for searching and returning the very first element
//  within the document that matches the given selector. The queryselector in javascript 
//  only returns the element that matches with one of the specified CSS selectors, 
...
// A simple example of an event is a user clicking the mouse or pressing a key on the keyboard.
...
// element or get the text content written inside that element. If you set the text using
//  textContent for an element, then the other child elements will be removed and only
//   this text will be added in that element.